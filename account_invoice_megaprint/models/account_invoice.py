...
class AccountInvoice(models.Model):
    ...
    xml_response_cancel = fields.Text(string='XML Response C', readonly=True, states={'draft': [('readonly', False)]}, copy=False)
    xml_notes = fields.Text('XML Children')
    uuid_refund = fields.Char('UUID a rectificar', related="invoice_refund_id.uuid")
    txt_filename = fields.Char('Archivo txt', required=False, readonly=True, copy=False)
    file = fields.Binary('Archivo', required=False, readonly=True, copy=False)
    txt_filename_xml = fields.Char('Archivo XML Nombre', required=False, readonly=True, copy=False)
    ...
    def onchange_frases(self):
        for record in self:
            if record.company_id and record.company_id.frase_ids:
                record.frase_ids = record.company_id.frase_ids.ids
    ...
    def action_post(self):
        ...
        for record in self:
            if record.move_type in ('out_invoice', 'out_refund') and record.journal_id.is_fel == True:
                if record.exportacion_fel:
                    xml = record.generate_xml_export()
                    _logger.info("**********FACTURA DE EXPORTACION**********")
                    _logger.info(xml.decode('utf-8'))
                    #raise UserError(('%s') %(xml))
                elif record.journal_id.is_nota_abono:
                    nabn_dict = record.generate_xml_nabn()
                    xml = record.GenerateXML_NABN(data=nabn_dict)
                    #raise UserError(('%s') %(xml))
                    _logger.info("**********NOTA DE ABONO**********")
                    _logger.info(xml.decode('utf-8'))

                else:
                    xml = record.generate_xml()
                    _logger.info("**********FACTURA**********")
                    _logger.info(xml.decode('utf-8'))
                #raise UserError(('%s') %(xml))
                xml_request = record.dte_request(xml_string=xml.decode('utf-8'), type_request='FirmaDocumentoRequest')
                #_logger.info(xml_request.decode('utf-8'))
                xml_sing = record.get_signature(xml_request.decode('utf-8'))
                #_logger.info(xml_sing.decode('utf-8'))
                xml_signed = record.dte_request(xml_string=xml_sing, type_request='RegistraDocumentoXMLRequest')
                #_logger.info(xml_signed.decode('utf-8'))
                record.register_dte(xml_signed.decode('utf-8'))
            elif (record.move_type == 'in_invoice' and record.journal_id.is_fel == True):
                if record.journal_id.is_factura_especial:
                    fe_dict = record.generate_xml_fe()
                    _logger.info(fe_dict)
                    xml = record.GenerateXML_FESP(data=fe_dict)
    ...
    def generate_xml(self):
        ...
        AdendaSummary.append(str(html2plaintext(self.narration) if self.narration else "")) #valor9 (MOSTRAR COMENTARIOS QUE SE AGREGUEN  EN EL CAMPO "NARRATION" DEL FORM DE  LA FACTURA EN EL PDF DE IMPRESIÓN)
    ...
    def post_dte(self, xml_request, type):
        ...
        if xml_request and type:
            ...
            try:
                response  = requests.post(post_url, data=xml_request, headers=headers, stream=True)
                return response
            except Exception as e:
                raise UserError(('%s') %(e))
            #finally:
    ...
    def onchange_nit(self):
        for record in self:
            if record.partner_vat:
                nit = record.partner_vat
                if '-' in nit:
                    record.partner_vat = nit.replace('-', '')
